...
* 캐시는 결과를 더 빠르게 제공하기 위해 데이터베이스나 다른 서비스까지의 불필요한 왕복을 제거한다. 잘 사용된다면 엄청난 성능 향상을 가져온다. 대규모 요청에도 HTTP가 확장을 매우 잘하는 이유는 캐싱의 개념이 내장되어 있기 때문이다. 분산 시스템에서는 일반적으로 캐싱을 클라이언트에 둘지 서버 측에 둘지 고민한다. 과연 어떤 것이 최선일까?
